...
def post(label, price, final_rate, taken):
    ...
    # Increment id_product after successful POST
    ...
    time.sleep(1)
    reset_values()  # Reset after each POST request
...
def rate(final_weight, label, taken):
    print("Calculating rate for", label)
    item_rates = {
        a: {"price": 10, "rate_per_gram": 0.01},
        b: {"price": 20, "rate_per_gram": 0.02},
        l: {"price": 1, "rate_per_gram": 1},   # fixed price for Lays
        c: {"price": 2, "rate_per_gram": 2},   # fixed price for Coke
    }

    if label in item_rates:
        rate_data = item_rates[label]
        final_rate = final_weight * rate_data["rate_per_gram"]
        post(label, rate_data["price"], final_rate, taken)
    else:
